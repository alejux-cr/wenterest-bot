...
module.exports = (params) => {
  const {
    witService, domainService, sessionService,
  } = params;
  ...
  const domainIntentHandler = {
    canHandle(handlerInput) {
      return handlerInput.requestEnvelope.request.type === 'IntentRequest'
        && handlerInput.requestEnvelope.request.intent.name === 'domain';
    },
    async handle(handlerInput) {
      ...
      if (!conversation.complete) {
        ...
        const domainResult = await domainService
          .tryDomain(domainName, userName);
        speechText = domainResult.success || domainResult;
      }
